...
recommendedValidation = \case
    ...
    RequireSomeOf m script -> do
        when (m == 0) $ Left MZero
        when (m > fromIntegral (maxBound @Int)) $ Left BigNInRequiredSomeOf
...
validateScriptTemplate level (ScriptTemplate cosigners_ script) = do
    ...
    when (level == RecommendedValidation) $
        check BigCosignerNumber (all (\(Cosigner n) -> n <= fromIntegral (maxBound @Int)) scriptCosigners)
  where
...
prettyErrValidateScript = \case
    ...
    NotRecommended BigNInRequiredSomeOf ->
        "The script requires more than maxBound Int signatures. Such scripts might result in transactions exceeding the possible transaction size limit (which is not recommended)."
...
prettyErrValidateScriptTemplate = \case
    ...
    BigCosignerNumber ->
        "The script template contains a cosigner number greater than maxBound Int. Such scripts might result in transactions exceeding the possible transaction size limit (which is not recommended)."
--
-- Internal
--
